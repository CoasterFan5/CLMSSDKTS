 //Generated by CoasterFan5
  const baseUrl = "/v1/courses/{course_id}/assignments/overrides";
  import type {AssignmentOverride} from "../models/AssignmentOverride"

  const method = "GET"

  const urlBuilder = (base: string, items: Record<string, unknown>) => {
    let newUrl = baseUrl
    for (const key in items) {
      newUrl = newUrl.replaceAll(`{${key}}`, `${items[key]}`)
    }

    return base + newUrl
  }

export const batch_retrieve_overrides_in_course = async (token: string, baseUrl: string, params: {
  "course_id": string;"assignment_overrides[id]": unknown;"assignment_overrides[assignment_id]": unknown
}) => {
  console.log(token)
  const r = await fetch(urlBuilder(baseUrl, params), {
    method: method,
    headers: {
      "Authorization": "Bearer " + token
    }
  })
  if(r.status != 200) {
    console.error(r)
    return undefined
  }
  return (await r.json()) as AssignmentOverride[]
}
